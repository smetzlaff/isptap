 ...
 ******************************************************************************/
#ifndef _FIFO_BF_DISP_STATE_MAINTAINER_HPP_
#define _FIFO_BF_DISP_STATE_MAINTAINER_HPP_

#include "bf_disp_state_maintainer.hpp"
#include "fifo_disp_mem_state.hpp"

/*!
 * \brief The abstract DISP state used by BFDISPStateMaintainerFIFO.
 * It is represented as a vector of FIFODISPMemState objects.
 */
typedef vector<FIFODISPMemState>  AbsFIFODISPMemState;

/*!
 * \brief A map to store the abstract DISP states for the BFDISPStateMaintainerFIFO for each MSGVertex node in the graph.
 * The abstract DISP state is represented by a pointer to a vector of concrete states.
 */
typedef map<MSGVertex, AbsFIFODISPMemState*> NodeFIFODISPStateMap;

/*!
 * \brief DISP state maintainer for brute force analysis supporting update() and join() for data flow analysis for the FIFO replacement policy.
 ...
 */ 
class BFDISPStateMaintainerFIFO : public BFDISPStateMaintainer {
	public:
		/*!
		 * \brief Constructor.
		 ...
		 */
		BFDISPStateMaintainerFIFO(disp_params_t params, FunctionCallGraphObject* fcgo);
		 /*!
	...
	private:
		 ...
		 * Made private.
		 */
		BFDISPStateMaintainerFIFO();
		/*!
		 * \brief Destructor.
		 */ 
		virtual ~BFDISPStateMaintainerFIFO();
		 ...
		 */
		void updateState(AbsFIFODISPMemState *state, uint32_t function_addr, activation_type_t activation, uint32_t previous_function);
		/*!
		 ...
		 */
		AbsFIFODISPMemState *getAbsFIFODISPMemState(MSGVertex node);
		/*!
		 * \brief Registers an abstract DISP state for a given node in the NodeMemSetMap.
		 * This is necessary to get the abstract DISP state later on using getAbsFIFODISPMemState().
		 ...
		 */
		bool addAbsMemState(MSGVertex node, AbsFIFODISPMemState *state);
		/*!
		 ...
		 */
		void removeDuplicates(AbsFIFODISPMemState *state);
		/*!
		 ...
		 */
		void printMemSet(ostringstream *os, AbsFIFODISPMemState *state);
		/*!
		 ...
		 * \returns True if the function is in all concrete DISP states, else false.
		 */
		bool isInAllSets(uint32_t addr, AbsFIFODISPMemState *state);
		/*!
		 ...
		 * \returns True if the function is in any concrete DISP states, else false.
		 */
		bool isInAnySet(uint32_t addr, AbsFIFODISPMemState *state);
		/*!
