 ...
 ******************************************************************************/
#include "disp_dfa.hpp"
#include "arch_cfg_factory.hpp"

LoggerPtr DISPDataFlowAnalyzer::logger(Logger::getLogger("DISPDataFlowAnalyzer"));

DISPDataFlowAnalyzer::DISPDataFlowAnalyzer(CFGMSGPair graph_pair, CFGMSGVPair entry_node, CFGMSGVPair exit_node, FunctionCallGraphObject *fcgo)
{
	graph = graph_pair;
	entry = entry_node;
	exit = exit_node;
...
DISPDataFlowAnalyzer::DISPDataFlowAnalyzer(CFGMSGPair graph_pair, CFGMSGVPair entry_node, CFGMSGVPair exit_node, FunctionCallGraphObject *fcgo, uint32_t mem_size)
{
	graph = graph_pair;
	entry = entry_node;
	exit = exit_node;
...
DISPDataFlowAnalyzer::~DISPDataFlowAnalyzer()
{
	ctxMap.clear();
	functions = NULL;
...
void DISPDataFlowAnalyzer::analyzeDISP(void)
{
	...
	while(processing.size() != 0)
	{
		...
		if(isPredecessorMemStateKnown(actual) && (actual != exit))
		{
			...
			if(logger->isDebugEnabled())
			{
				...
				s1 << "Must set: ";
				disp_state_maintainer->printMemSet(&s1, &state.must_set);
				LOG_DEBUG(logger, s1.str());

				s2 << "May set: ";
				disp_state_maintainer->printMemSet(&s2, &state.may_set);
...
void DISPDataFlowAnalyzer::categorizeMemAccesses(void)
{
...
//			if(mv != entry.second)
			{
				...
				bool in_must = disp_state_maintainer->isInSet(function_addr, &mem.must_set);
				bool in_may = disp_state_maintainer->isInSet(function_addr, &mem.may_set);
				if(in_must)
				{
					disp_call_hit++;
				}
...
void DISPDataFlowAnalyzer::calculateMemPenalty(void)
{
...
//			if(mv != entry.second)
			{
				...
				LOG_DEBUG(logger, "DISP penalty for entry node: " << mv << " called function with address 0x" << hex << function_addr << " with size: " << dec << functions->getFunctionSize(function_addr) << "/" << disp_state_maintainer->getFunctionMemSize(function_addr) << " is " << dec << disp_cost << " " << ((in_must)?("[HIT]"):("")) << ((!in_must && in_may)?("[NC]"):("")) << ((!in_must && !in_may)?("[MISS]"):("")));
...
//			if(mv != exit.second)
			{
				...
				else
				{
					...
					LOG_DEBUG(logger, "DISP penalty for exit node: " << mv << " returning to function with address 0x" << hex << function_addr << " with size: " << dec << functions->getFunctionSize(function_addr) << "/" << disp_state_maintainer->getFunctionMemSize(function_addr) << " is " << dec << disp_cost << " " << ((in_must)?("[HIT]"):("")) << ((!in_must && in_may)?("[NC]"):("")) << ((!in_must && !in_may)?("[MISS]"):("")));
				}
			}
		}
	}
...
bool DISPDataFlowAnalyzer::isPredecessorMemStateKnown(CFGMSGVPair node)
{
...
void DISPDataFlowAnalyzer::joinAndSetInitialMemState(CFGMSGVPair node)
{
	msgInEdgeIter ep;
	...
	for(ep = in_edges(node.second, graph.second); ep.first != ep.second; ++ep.first) 
	{
		...
		if(get(msgEdgeTypeEProp, e) != BackwardJump)
		{
			...
			abs_mem_set_t tmp = get(memStateNProp, predecessor_m);
	...
	abs_mem_set_t new_state = disp_state_maintainer->join(&prev_states, (getContextForNode(node).top()));
	put(memStateNProp, node.second, new_state);
	put(memStateValNProp, node.second, true);

}

void DISPDataFlowAnalyzer::setInitialMemState(CFGMSGVPair node)
{
	if(node == entry)
	{
		abs_mem_set_t initial_state;
		disp_state_maintainer->setBlankMemState(&initial_state.must_set, &initial_state.may_set);
	...
	else
	{
		...
		abs_mem_set_t prev_state = get(memStateNProp, predecessor_msg);
		...
		//	if(predecessor != entry) // the super entry is ignored
			{
				...
				prev_state = disp_state_maintainer->update(&prev_state, getFunctionAddress(predecessor), predecessor_type, getContextForNode(predecessor).top());
			}
		}
		else if(predecessor_type == Exit)
		{
			...
			if(!ctx.empty())
			{
				...
				prev_state = disp_state_maintainer->update(&prev_state, ctx.top(), predecessor_type, callee);
			}
			else
			{
...
ContextStack DISPDataFlowAnalyzer::getContextForNode(MSGVertex mnode)
{
	ContextStack context;
...
ContextStack DISPDataFlowAnalyzer::getContextForNode(CFGMSGVPair node)
{
	return getContextForNode(node.second);
}


ContextStack DISPDataFlowAnalyzer::updateContextForNode(CFGMSGVPair node)
{
	ContextStack context;
...
void DISPDataFlowAnalyzer::addContextForNode(CFGMSGVPair node, ContextStack context)
{
	NodeContextMap::iterator pos;
	bool ins_bool;
...
uint32_t DISPDataFlowAnalyzer::getFunctionAddress(CFGVertex cnode)
{
...
uint32_t DISPDataFlowAnalyzer::getFunctionAddress(CFGMSGVPair node)
{
	return getFunctionAddress(node.first);
}

uint32_t DISPDataFlowAnalyzer::getForwardInEdgeCount(MSGVertex v)
{
	msgInEdgeIter ep;
...
void DISPDataFlowAnalyzer::setMemParameters()
{
	...
	switch(disp_parameters.rpol)
	{
		case LRU:
			{
				LOG_DEBUG(logger, "Setting up DISP abstract state maintainer with LRU as replacement policy");
				disp_state_maintainer = new DISPStateMaintainerLRU(disp_parameters, functions);
				break;
			}
		default:
			{
...
uint32_t DISPDataFlowAnalyzer::getDispHitPenalty(bool call)
{
...
uint32_t DISPDataFlowAnalyzer::getDispMissAndLoadPenalty(uint32_t function_size_in_mem, activation_type_t act)
{
	// the minimal call/return latency is considered here (when memory is ONCHIP, i.e. getCallLatency(true) and getReturnLatency(true)) to maximise the penalty
...
uint64_t DISPDataFlowAnalyzer::getMemoryStateCount(void)
{
	return disp_state_maintainer->getMemoryStateCount();
}

uint64_t DISPDataFlowAnalyzer::getRepresentationStateCount(void)
{
	return disp_state_maintainer->getRepresentationStateCount();
}

uint64_t DISPDataFlowAnalyzer::getUsedMemSize(void)
{
	return disp_state_maintainer->getUsedSize();
}

uint64_t DISPDataFlowAnalyzer::getUsedMemReferences(void)
{
